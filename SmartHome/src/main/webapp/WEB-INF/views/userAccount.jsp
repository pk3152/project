		...
		<header id="header">
			<div class="content">
				...
				<table>
					...
					<c:forEach var="item" items="${list}" varStatus="val">
					<tr>
							<%-- <span class="uid"><a href="adminView?id=${item.userID}">${item.userID}</a></span> --%>
						<td><a href="/SmartHome/adminView?id=${item.userID}"><input type="text" class="pointer" style="background: transparent;" value="${item.userID}" readonly size="5"></a></td>
						<td><input type="text" style="background: transparent;" class="pointer" value="${item.userName}" readonly size="5"></td>
						<td><input type="text" style="background: transparent;" class="pointer" value="${item.userEmail}" readonly></td>
						<td><input type="text" style="background: transparent;" class="pointer" value="${item.joinDate}" readonly></td>
						<td><input type="text" style="background: transparent;" class="pointer" value="IP" readonly></td>
					</tr>
					</c:forEach>
				</table>	
				</div>
		...
		<!-- userView 회원정보 페이지  -->
			<article id="adminView">
				...
				<form method="post" action="/SmartHome/update">
					...
					<div class="field">
						<label for="email">배당 IP</label>
						<input type="text" name="userIP" id="userIP" value=""style="background: transparent;" required />
					</div>
					<input type="hidden" name="userID" id="userID" value="${uID}" />
					<ul class="actions" style="text-align:right">
						<li><input type="button" value="회원탈퇴" onclick="location.href='/SmartHome/userDelete?userID=${uID}'" /></li>
